 ...
 * {@link CallableStatementCreator} objects with different parameters based on a
 * SQL statement and a single set of parameter declarations.
 * 
 * @author Rod Johnson
...
public class CallableStatementCreatorFactory {
     ...
     * ResultSet. specific type of ResultSet.
     * 
     * @param resultSetType
     *            the ResultSet type
    ...
    private class CallableStatementCreatorImpl implements CallableStatementCreator, SqlProvider {
        ...
        public CallableStatement createCallableStatement(Connection con) throws SQLException {
            if (this.inParameters == null) {
                throw new DataAccessException("A Map of parameters must be provided");
            }
            ...
            for (SqlParameter declaredParam : declaredParameters) {
                if (!declaredParam.isResultsParameter()) {
                    ...
                    if (declaredParam instanceof ResultSetSupportingSqlParameter) {
                        // It's an output parameter: SqlReturnResultSet
                        // parameters already excluded.
                        ...
                        // It's an input parameter; must be supplied by the
                        // caller.
                        if (!this.inParameters.containsKey(declaredParam.getName())) {
                            throw new DataAccessException("Required input parameter '" + declaredParam.getName()
                                    + "' is missing");
                        }
                        StatementCreatorUtils.setParameterValue(cs, sqlColIndx, declaredParam, inValue);
                    }
