	...
	public void dependInject(Object object, Bean bean) throws IllegalArgumentException, NoSuchBeanDefinitionException,
		...
		for (Method method : clazz.getMethods()) {
			if (method.getName().startsWith("set")) {
				...
				String injectName = CaseFormat.UPPER_CAMEL.to(CaseFormat.LOWER_CAMEL, method.getName().substring(3));
				Object value = bean.getParam(injectName);
				if (value != null && injectType.isAssignableFrom(value.getClass())) {
				...
				if (inject != null) {
					...
					// 注入
					Object injectBean = getInjectBean(inject, injectName, injectType);
					checkNotNull(injectBean, "inject target [%s(%s)] not found", injectName, injectType.getName());
