...
public interface KTableModel {
	 ...
	 */
	Object getContentAt(int col, int row, boolean show);
	...
	/**
	 * This function tells the KTable how many columns form a fixed region that
	...
	/**
	 * 当选择一个单元格的时候，可能不在有效行数内，这时候由model来判断是否需要新增行
	 * @param col
	 * @param row
	 * @return
	 */
	boolean needNewRow(int row);

	/**
	 * 选择单元格时候，是不是应该跳过而不被选中，由model说了算
	 * @param col
	 * @param row
	 * @return
	 */
	boolean skipFocusCol(int col, int row);
}