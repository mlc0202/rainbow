 ...
 * <code>KTableSortedModel</code> that provides a transparent sorting of cells.<br>
 ...
 * <p>
 * When drawing a cell, the table calls a <code>KTableCellRenderer</code> to do
 * this work. The table model determines which cell renderer is used for which
 * cell. A default renderer is available (
 * <code>KTableCellRenderer.defaultRenderer</code>), but the creation of
 ...
 * <code>KTableCellEditor</code>. Again the KTable asks the model to provide an
 * implementation. Note that there are multiple celleditors available in the
...
public class KTable extends Canvas {
	 ...
	 * <li><b>SWT.H_SCROLL</b> - show horizontal scrollbar and allow horizontal
	 * scrolling by arrow keys</li>
	 ...
	 * <li><b>SWTX.EDIT_ON_KEY</b> - Activates a possibly present cell editor on
	 * every keystroke. (Default: only ENTER). However, note that editors can
	 ...
	 * looses focus.</li> After creation a table model should be added using
	 * setModel().
	 */
	public KTable(Composite parent, int style) {
		...
		cellSelectionListeners = new ArrayList<KTableCellSelectionListener>();
		cellDoubleClickListeners = new ArrayList<KTableCellDoubleClickListener>();
		cellResizeListeners = new ArrayList<KTableCellResizeListener>();
			...
			private void repaintRelevantCells(GC gc, Point[] selections) {
				...
				for (Point point : selections) {
					...
					for (int j = 0; j < getModel().getFixedHeaderColumnCount(); j++) {
						...
						Rectangle rowClip = new Rectangle(1, 1 + fixedHeight, fixedWidth, bounds.height - 1
								- fixedHeight);
	...
	protected Rectangle getCellRectIgnoreSpan(int col, int row, int x_startValue) {
		...
		// T O D O: Make lines seperately configured and seperately drawn items
	...
	protected void drawBottomSpace(GC gc) {
		...
		if ((getStyle() & SWTX.FILL_WITH_DUMMYCOL) != 0) {
			...
			for (int row = 0; row < getFixedRowCount(); row++) {
				...
				fixedRenderer.drawCell(gc, new Rectangle(lastColRight + 2, ystart, r.width - 1, rowheight - 1), -1, -1,
						"", false, true, false, m_Model);
			...
			for (int row = m_TopRow; row < m_TopRow + m_RowsVisible + 1; row++) {
				int rowHeight = m_Model.getRowHeight(row);
				defaultRenderer.drawCell(gc, new Rectangle(lastColRight + 2, ystart, r.width - 1, rowHeight - 1), -1,
						-1, "", false, false, false, m_Model);
				ystart += rowHeight;
			}
			...
			if (m_Model.getRowCount() > 0) {
				...
				if (m_LeftColumn + m_ColumnsVisible == m_Model.getColumnCount())
					gc.drawLine(clientArea.x + clientArea.width - 1, 0, clientArea.x + clientArea.width - 1,
	 ...
	 * Model.belongsToCell()</code> that are on the right or below the given
	 *             cell.
	...
	protected void drawCell(GC gc, int col, int row, Rectangle rect) {
		...
		m_Model.getCellRenderer(col, row).drawCell(gc, rect, col, row, m_Model.getContentAt(col, row, true),
				showAsSelected(col, row) || highlightSelectedRowCol(col, row), isHeaderCell(col, row),
	...
	public Point calcColumnNum(int x, int y) {
		...
		if (found != null)
			return found;
		found = checkIfMatchesInColumns(valid.y, m_TopRow + m_RowsVisible, valid.x, m_LeftColumn + m_ColumnsVisible,
				toFind, true);
		if (found != null)
			return found;
	 ...
	 */
	protected Point checkIfMatchesInColumns(int startRow, int endRow, int startCol, int endCol, Point toFind,
			boolean span) {
	...
	protected void focusCell(int col, int row, int stateMask) {
		if (m_Model.skipFocusCol(col, row))
			return;
				...
				if (isRowSelectMode()) {
					...
					if (sel != null)
						for (Point element : sel) {
							...
							if (element.y < min.y)
								min.y = element.y;
							if (!containsCell && element.x == col && element.y == row)
								containsCell = true;
						}
	...
	protected boolean clickInSelectedCells(Point click) {
		...
		if (selection == null)
			return false;
		for (Point element : selection) {
			if (getCellRect(element.x, element.y).contains(click))
				return true;
		}
		return false;
	}
			...
			if (m_ResizeColumnIndex != -1 || m_ResizeRowIndex != -1) {
				...
				if (isHeaderCell(cell.x, cell.y)) {
					...
					if (editor != null
							...
							&& editor.isApplicable(KTableCellEditor.SINGLECLICK, this, cell.x, cell.y, click, null,
									e.stateMask)) {
						int oldFocusCol = m_FocusCol;
						int oldFocusRow = m_FocusRow;
						m_FocusCol = cell.x;
					...
					if (editor != null
							&& rect.contains(click)
							&& (editor.getActivationSignals() & KTableCellEditor.SINGLECLICK) != 0
							&& editor.isApplicable(KTableCellEditor.SINGLECLICK, this, m_FocusCol, m_FocusRow, click,
									null, e.stateMask)) {
						openEditorInFocus();
					}
				}
	...
	protected void onTraverse(TraverseEvent e) {
		...
		if (newFocusRow == -1 || newFocusCol == -1) {
			newFocusRow = m_Model.getFixedHeaderRowCount();
			newFocusCol = m_Model.getFixedHeaderColumnCount();
			while ((newFocusCol < m_Model.getColumnCount()) && m_Model.skipFocusCol(newFocusCol, newFocusRow))
				newFocusCol++;
		} else if (e.keyCode == SWT.ARROW_LEFT || e.detail == SWT.TRAVERSE_TAB_PREVIOUS) {
			e.stateMask &= ~SWT.SHIFT;
			if (!isRowSelectMode()) {
				Point current = m_Model.belongsToCell(m_FocusCol, m_FocusRow);
				if (current == null)
					current = new Point(m_FocusCol, m_FocusRow);
				newFocusCol = current.x - 1;
				while (newFocusCol > m_Model.getFixedHeaderColumnCount()
						&& m_Model.skipFocusCol(newFocusCol, newFocusRow)) {
					newFocusCol--;
				}
				if (newFocusCol < m_Model.getFixedHeaderColumnCount() && newFocusRow > m_Model.getFixedHeaderRowCount()
		...
		} else if (e.keyCode == SWT.ARROW_RIGHT || e.detail == SWT.TRAVERSE_TAB_NEXT) {
			if (!isRowSelectMode()) {
				...
				if (newFocusCol >= m_Model.getColumnCount() && e.detail == SWT.TRAVERSE_TAB_NEXT) {
					if (m_FocusRow == m_Model.getRowCount() - 1) {
						// �����ɲ���������һ��
						if (m_Model.needNewRow(newFocusRow + 1))
							redraw();
					}
					if (newFocusRow < m_Model.getRowCount() - 1) {
						newFocusCol = m_Model.getFixedHeaderColumnCount();
						newFocusRow++;
						while ((newFocusCol < m_Model.getColumnCount())
								&& m_Model.skipFocusCol(newFocusCol, newFocusRow)) {
							newFocusCol++;
						}
					}
			...
			if (m_Model.needNewRow(newFocusRow))
				redraw();
		...
		if (newFocusCol >= m_Model.getFixedHeaderColumnCount() && newFocusCol < m_Model.getColumnCount()
				&& newFocusRow >= m_Model.getFixedHeaderRowCount() && newFocusRow < m_Model.getRowCount()) {
	...
	protected void onKeyDown(KeyEvent e) {
		...
		if ((e.character == ' ') || (e.character == '\r')) {
			...
			if (editor != null
					...
					&& editor.isApplicable(KTableCellEditor.KEY_RETURN_AND_SPACE, this, m_FocusCol, m_FocusRow, null,
							e.character + "", e.stateMask)) {
				openEditorInFocus();
				return;
			}
		}
		...
		if (e.keyCode == SWT.HOME) {
			...
			if (newFocusRow == -1)
				newFocusRow = m_Model.getFixedHeaderRowCount();
			newFocusCol = m_Model.getColumnCount() - 1;
			while (newFocusCol >= m_Model.getFixedHeaderColumnCount() && m_Model.skipFocusCol(newFocusCol, newFocusRow)) {
				newFocusCol--;
			}
			focusChanged = true;
			...
			if (e.keyCode == SWT.DEL || e.keyCode == SWT.BS) {
				...
				if (editor != null
						...
						&& editor.isApplicable(KTableCellEditor.KEY_ANY, this, m_FocusCol, m_FocusRow, null,
								e.character + "", e.stateMask)) {
					openEditorInFocus();
					if (m_CellEditor != null) {
	...
	protected void onMouseDoubleClick(MouseEvent e) {
		...
		if (e.button == 1) {
			...
			if (e.y < getHeaderHeight()) {
				...
				m_ResizeColumnIndex = -1;
				Point cell = getCellForCoordinates(e.x, e.y);
				fireFixedCellDoubleClicked(this, cell.x, cell.y, e.stateMask);
				return;
			} else {
				Point click = new Point(e.x, e.y);
				Point cell = getCellForCoordinates(e.x, e.y);
				if (isHeaderCell(cell.x, cell.y)) {
					...
					if (editor != null
							...
							&& editor.isApplicable(KTableCellEditor.DOUBLECLICK, this, cell.x, cell.y, click, null,
									e.stateMask)) {
						int oldFocusCol = m_FocusCol;
						int oldFocusRow = m_FocusRow;
						m_FocusCol = cell.x;
					...
					if (editor != null
							&& rect.contains(click)
							&& (editor.getActivationSignals() & KTableCellEditor.DOUBLECLICK) != 0
							&& editor.isApplicable(KTableCellEditor.DOUBLECLICK, this, m_FocusCol, m_FocusRow, click,
									null, e.stateMask))
						openEditorInFocus();
					fireCellDoubleClicked(this, cell.x, cell.y, e.stateMask);
				}
			}
		}
	}
	...
	class TooltipListener implements Listener {
		...
		public void handleEvent(Event event) {
			...
			case SWT.MouseHover: {
				...
				if (((tooltip == null || tooltip.equals("")) && (m_nativTooltip == null || m_nativTooltip.equals("")))
				...
				// T O D O: Correctly position the tooltip below the cursor.
		...
		if (column >= 0 && column < m_Model.getColumnCount()) {
			...
			for (int i = 0; i < m_Model.getFixedHeaderRowCount(); i++) {
				int width = m_Model.getCellRenderer(column, i).getOptimalWidth(gc, column, i,
						m_Model.getContentAt(column, i, true), true, m_Model);
				if (width > optWidth)
					optWidth = width;
			}
	...
	protected void fireCellSelection(int col, int row, int statemask) {
		for (KTableCellSelectionListener listener : cellSelectionListeners)
			listener.cellSelected(this, col, row, statemask);
	}

	protected void fireCellDoubleClicked(KTable table, int col, int row, int statemask) {
		for (KTableCellDoubleClickListener listener : cellDoubleClickListeners)
			listener.cellDoubleClicked(table, col, row, statemask);
	}

	protected void fireFixedCellDoubleClicked(KTable table, int col, int row, int statemask) {
		for (KTableCellDoubleClickListener listener : cellDoubleClickListeners)
			listener.fixedCellDoubleClicked(table, col, row, statemask);
	}

	protected void fireFixedCellSelection(int col, int row, int statemask) {
		for (KTableCellSelectionListener listener : cellSelectionListeners)
			listener.fixedCellSelected(this, col, row, statemask);
	}

	protected void fireColumnResize(int col, int newSize) {
		for (KTableCellResizeListener listener : cellResizeListeners)
			listener.columnResized(col, newSize);
	}

	protected void fireRowResize(int row, int newSize) {
		for (KTableCellResizeListener listener : cellResizeListeners)
			listener.rowResized(row, newSize);
	}
	...
	public void setSelection(int col, int row, boolean scroll) {
		checkWidget();
		if (m_Model.needNewRow(row))
			redraw();
		if (col < m_Model.getColumnCount() && col >= m_Model.getFixedHeaderColumnCount() && row < m_Model.getRowCount()
	 ...
	 * 
	 * @param selections
	 *            The selected cells as points. If <code>null</code> or an empty
		...
		if (selections == null || selections.length < 1) {
			...
			try {
				...
				for (int i = 0; i < selections.length; i++) {
					...
					if (col < m_Model.getColumnCount() && col >= m_Model.getFixedHeaderColumnCount()
							&& row < m_Model.getRowCount() && row >= m_Model.getFixedHeaderRowCount())
						if (i == 0)
	 ...
	 * in the x fields, rows in y fields. Returns null if in Row Selection Mode. <br>
		...
		if (sel != null) {
			for (Point element : sel) {
				if (element.x == col || element.y == row)
					return true;
				Point valid = getValidCell(element.x, row);
				if (valid.x == col)
					return true;
				valid = getValidCell(col, element.y);
				if (valid.y == row)
					return true;
			}
		}
	 ...
	 *            <li><b>SWTX.FILL_WITH_DUMMYCOL</b> - Makes the table fill any
	...
	public Color getBackground() {
		return m_Display.getSystemColor(SWT.COLOR_WHITE);
	}
}