...
//	}
	...
	protected class KTableCopyAllAction extends Action {
		...
		private Point[] getAllTableCells() {
			...
			if (model == null)
				return new Point[] {};
			List<Point> cells = new ArrayList<Point>(model.getColumnCount() * model.getRowCount());
		...
		protected void removeContentAt(Point[] selection) {
			...
			try {
				...
				for (Point element : selection) {
					model.setContentAt(element.x, element.y, "");
					if (updateSeperateCells)
						m_table.updateCell(element.x, element.y);
				}
			} finally {
				if (!updateSeperateCells)
	...
	protected class KTableSelectAllAction extends Action {
		...
		protected void selectAll(KTableModel model) {
			List<Point> sel = new ArrayList<Point>();
			...
			for (int col = topLeft.x; col <= bottomRight.x; col++) {
				...
				if (model.belongsToCell(col, row).equals(new Point(col, row))) {
					...
					if (nextValidCell.x == col && nextValidCell.y == row) {
						...
						Object content = model.getContentAt(col, row, true);
	...
	private String maskHtmlChars(String text) {
		...
		text = text.replaceAll("\344", "&auml;");
		text = text.replaceAll("\304", "&Auml;");
		text = text.replaceAll("\366", "&ouml;");
		text = text.replaceAll("\326", "&Ouml;");
		text = text.replaceAll("\374", "&uuml;");
		text = text.replaceAll("\334", "&Uuml;");
		text = text.replaceAll("\337", "&szlig;");
		text = text.replaceAll("\"", "&quot;");
	...
	protected String getTextForSelection(Point[] selection) {
		...
		for (int i = 0; i < selection.length; i++) {
			...
			Object content = model.getContentAt(selection[i].x, selection[i].y, true);
	...
	protected Point sortSelectedCells(Point[] selection) {
		Arrays.sort(selection, new Comparator<Point>() {
