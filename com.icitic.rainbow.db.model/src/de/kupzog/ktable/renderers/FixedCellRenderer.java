 ...
 * <li><b>INDICATION_SORT</b> if a sort indicator should be painted. Has only an
 * effect when the <code>KTableModel</code> used is an instance of
 * <code>KTableSortedModel</code>. <br>
 * Can be combined with <code>STYLE_FLAT</code> or <code>STYLE_PUSH</code> by
 * or-ing.</li>
...
public class FixedCellRenderer extends DefaultCellRenderer {
	 ...
	 * drawCellContent()
	 */
	protected void drawCellContent(GC gc, Rectangle rect, int col, int row, Object content, KTableModel model,
		...
		if ((m_Style & INDICATION_SORT) != 0 && model instanceof KTableSortedModel
				&& ((KTableSortedModel) model).getSortColumn() == col && model.getFixedHeaderRowCount() > row
				&& row == 0) {
		...
		drawCellContent(gc, rect, content == null ? "" : content.toString(), null, fgColor, bgColor);

		// draw sort indicator:
		if (indicator != null) {
			gc.fillRectangle(rect.x + rect.width, rect.y, Math.min(indicator.getBounds().width + 8, oldWidth
