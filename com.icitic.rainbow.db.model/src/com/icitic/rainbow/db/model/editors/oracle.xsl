...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:r="http://www.icitic.com/core/db/model">
	...
	<xsl:template match="r:entity" mode="create">
		...
		<xsl:if test="r:columns/r:column[r:key = 'true']">
			...
			<xsl:choose>
				<xsl:when test="indexSpace">
					<xsl:text>) using index tablespace </xsl:text>
					<xsl:value-of select="r:indexSpace"/>
				</xsl:when>
				<xsl:otherwise>) </xsl:otherwise>
			</xsl:choose>				
		</xsl:if>
		<xsl:choose>
			<xsl:when test="tableSpace">
				<xsl:text>&#xA;) tablespace </xsl:text>
				<xsl:value-of select="r:tableSpace"/>
				<xsl:text>;&#xA;&#xA;</xsl:text>
			</xsl:when>
			<xsl:otherwise>&#xA;) ;&#xA;&#xA;</xsl:otherwise>
		</xsl:choose>	
